...
void unknowncheats::legit_t::run_aimbot(const c_entity& entity, const c_entity& local, vec3_t local_pos, int ent_idx, int local_idx) {
    ...
    vec3_t opp_pos;
    int screen_center_x = GetSystemMetrics(SM_CXSCREEN) / 2;
    int screen_center_y = GetSystemMetrics(SM_CYSCREEN) / 2;
    ...
    distance_to_sight = entity.get_bone().bone_pos_list[bone_index::head].screen_pos.dist_to({ static_cast<float>(screen_center_x), static_cast<float>(screen_center_y) });
